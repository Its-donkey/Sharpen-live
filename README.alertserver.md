...
1. Start the HTTP server:
   ...
   ```
2. Streamer data is appended to `data/streamers.json`. Provide a different path through `CreateOptions.FilePath` if you embed the handler elsewhere. Serve the UI with `go run ./cmd/ui-serve -dir ui ...` (or `./cmd/ui-server` for SSR) so API traffic and UI assets stay decoupled.

## Companion UI (alGUI)
The UI source and assets live in `ui/` with entrypoints under `cmd/ui-*`. Build `ui/main.wasm` via `GOOS=js GOARCH=wasm go build -o ui/main.wasm ./cmd/ui-wasm`, then serve it locally with `go run ./cmd/ui-serve -dir ui -api http://127.0.0.1:8880` (or use `cmd/ui-server` if you prefer server-rendered HTML). Both binaries share the same module so no extra checkout is needed.
