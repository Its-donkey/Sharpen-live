(function () {
  ...
  let inflightTarget = '';
  let debounceTimer;
  let addLanguageCallback = null; // Will be set by initLanguagePicker
  ...
  async function fetchMetadata(target) {
    ...
    try {
      ...
      // Auto-fill languages (append to existing selections)
      if (data.languages && Array.isArray(data.languages) && addLanguageCallback) {
        data.languages.forEach((lang) => {
          if (lang && lang.trim() !== '') {
            // addLanguageCallback expects (value, label) - use same for both
            addLanguageCallback(lang.trim(), lang.trim());
          }
        });
      }
      // Store channel ID for WebSub/Webhook setup
      if (data.channelId) {
  ...
  function bindPlatformInputs() {
    ...
    // Only bind metadata handlers to the FIRST input
    const firstInput = inputs[0];
    log('binding metadata handlers to first platform input only');

    const firstRow = firstInput.closest('.platform-row');
    const firstSelect = firstRow ? firstRow.querySelector('.platform-select select, .platform-select-input') : null;
    const firstSelectWrapper = firstRow ? firstRow.querySelector('.platform-select') : null;
    const metadataHandler = (e) => queueMetadataFetch(e.target.value || '');
    const visibilityHandler = () => updatePlatformVisibility(firstInput, firstSelectWrapper);

    firstInput.addEventListener('blur', metadataHandler);
    firstInput.addEventListener('change', metadataHandler);
    ...
    if (firstSelect) {
      firstSelect.addEventListener('change', () => {
        ...
        const url = platformURLForHandle(firstSelect.value, current);
        if (!url) return;
    ...
    // Bind platform select handlers to ALL other inputs (for @handle conversion)
    for (let i = 1; i < inputs.length; i++) {
      ...
      const visHandler = () => updatePlatformVisibility(input, selectWrapper);

      input.addEventListener('input', visHandler);
    ...
    // Trigger initial metadata fetch if first input has value
  ...
  function initLanguagePicker() {
    ...
    // Expose addLanguage to the outer scope for metadata auto-fill
    addLanguageCallback = addLanguage;
  ...
  function init() {
    initLanguagePicker(); // Initialize first to set addLanguageCallback
